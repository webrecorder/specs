...
class TestWaczFormat(unittest.TestCase):
    ...
    def setUpClass(self):   
        ...
        with zipfile.ZipFile(os.path.join(TEST_DIR, 'valid_example_2.wacz'), "r") as zip_ref:
            zip_ref.extractall("tests/fixtures/unzipped_wacz_2")
            zip_ref.close()
            
        self.validation_class_valid_2 = Validation(os.path.join(TEST_DIR, 'valid_example_2.wacz'))
        self.validation_class_valid_1 = Validation(os.path.join(TEST_DIR, 'valid_example_1.wacz'))
        self.validation_class_valid_0 = Validation(os.path.join(TEST_DIR, 'valid_example_0.wacz'))
        self.validation_class_invalid = Validation(os.path.join(TEST_DIR, 'invalid_example_1.wacz'))


    def test_overall_command(self):
        self.assertTrue(main(['validate', '-f', os.path.join(TEST_DIR, 'valid_example_0.wacz')]))
